...
import { useCallback, useEffect, useMemo, useState, type FC, type Key } from 'react';
...
const secondaryBtnClass =
  'bg-neutral-100 text-neutral-600 px-3 py-1 text-xs rounded border border-neutral-200 hover:bg-neutral-200 hover:text-neutral-800 pressed:bg-neutral-50 outline-none focus-visible:ring-1 focus-visible:ring-sky-300 cursor-pointer transition-colors';
const dangerBtnClass =
  'bg-red-50 text-red-600 px-3 py-1 text-xs rounded border border-red-200 hover:bg-red-100 pressed:bg-red-50 outline-none focus-visible:ring-1 focus-visible:ring-red-400 cursor-pointer transition-colors';
const sectionTitleClass = 'text-[11px] font-medium tracking-widest text-neutral-400';
const primaryBtnClass =
  'bg-sky-500 text-white px-4 py-1 text-xs font-medium rounded hover:bg-sky-400 pressed:bg-sky-600 outline-none focus-visible:ring-1 focus-visible:ring-sky-300 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer transition-colors';
...
const ModelForm: FC<{
  ...
  const handleName = useCallback((v: string) => setForm((prev) => ({ ...prev, name: v })), []);
  const handleVoiceId = useCallback((v: string) => setForm((prev) => ({ ...prev, voiceId: v })), []);
  const handleSystemPrompt = useCallback((v: string) => setForm((prev) => ({ ...prev, systemPrompt: v })), []);
  const handleSubmit = useCallback(() => onSubmit(form), [onSubmit, form]);
  ...
  return (
    <div className="space-y-3 bg-neutral-50 border border-neutral-200 p-4 rounded">
      <TextField value={form.name} onChange={handleName}>
        <Label className={labelClass}>モデル名</Label>
        <Input className={inputClass} placeholder="例: ナレーター、キャラA" />
      </TextField>

      <div>
        <TextField value={form.voiceId} onChange={handleVoiceId}>
        ...
        <p className={hintClass}>
          ...
          で声を検索し、⋯ から「Copy ID」で取得できます
        </p>
      </div>

      <div>
        <TextField value={form.systemPrompt} onChange={handleSystemPrompt}>
      ...
      <div className="flex gap-2 pt-1">
        <Button onPress={handleSubmit} isDisabled={!isValid} className={primaryBtnClass}>
          {submitLabel}
        </Button>
...
interface PresetItemProps {
  ...
  onCancelEdit: () => void;
  onUpdate: (id: string, form: ModelFormState) => void;
  onDelete: (id: string) => void;
}

const PresetItem: FC<PresetItemProps> = ({ id, name, voiceId, systemPrompt, isEditing, onEdit, onCancelEdit, onUpdate, onDelete }) => {
  const initial = useMemo(() => ({ name, voiceId, systemPrompt }), [name, voiceId, systemPrompt]);
  const handleUpdate = useCallback((form: ModelFormState) => onUpdate(id, form), [onUpdate, id]);
  const handleEdit = useCallback(() => onEdit(id), [onEdit, id]);
  const handleDelete = useCallback(() => onDelete(id), [onDelete, id]);

  if (isEditing) {
    return <ModelForm initial={initial} onSubmit={handleUpdate} onCancel={onCancelEdit} submitLabel="更新" />;
  }

  return (
    <div className="flex items-center justify-between bg-white border border-neutral-200 px-4 py-3 rounded">
      <div className="min-w-0">
        <p className="text-sm text-neutral-800 truncate">{name}</p>
        <p className="text-xs text-neutral-400 truncate">{voiceId}</p>
      </div>
      <div className="flex gap-2 shrink-0 ml-3">
        <Button onPress={handleEdit} className={secondaryBtnClass}>
          編集
        </Button>
        <Button onPress={handleDelete} className={dangerBtnClass}>
          削除
        </Button>
      </div>
    </div>
...
export const SettingsPage: FC = () => {
  ...
  const handleCartesiaApiKey = useCallback((v: string) => handleChange('cartesiaApiKey', v), [handleChange]);
  const handleAnnotate = useCallback((v: boolean) => handleChange('annotate', v), [handleChange]);
  const handleAnthropicApiKey = useCallback((v: string) => handleChange('anthropicApiKey', v), [handleChange]);
  const handleModelChange = useCallback((key: Key) => handleChange('model', String(key)), [handleChange]);
  const handleLanguageChange = useCallback((key: Key) => handleChange('language', String(key)), [handleChange]);
  const handleSampleRateChange = useCallback((key: Key) => handleChange('sampleRate', Number(key)), [handleChange]);
  ...
  return (
    <div className="max-w-lg space-y-8">
      ...
      <section className="space-y-5">
        ...
        <div>
          <TextField value={settings.cartesiaApiKey} onChange={handleCartesiaApiKey} type="password">
        ...
        <div className="space-y-3">
          <Checkbox isSelected={settings.annotate} onChange={handleAnnotate} className="group flex items-center gap-2.5 text-sm text-neutral-500 cursor-pointer outline-none">
          ...
          <p className="text-xs text-neutral-400 ml-[26px]">テキストを AI が分析し、嬉しい・悲しい・驚きなどの感情タグを自動で付けます。 より自然で表現力豊かな音声が生成されます。</p>
        </div>

        {settings.annotate && (
          <div>
            <TextField value={settings.anthropicApiKey} onChange={handleAnthropicApiKey} type="password">
      ...
      <section className="space-y-5">
        <div>
          ...
          <p className="text-xs text-neutral-400 mt-1">キャラクターや用途ごとに声と読み方を設定できます</p>
        </div>
        ...
        {showAddForm ? (
          <ModelForm initial={emptyForm} onSubmit={handleAddPreset} onCancel={handleCancelAddForm} submitLabel="追加" />
        ) : (
          <Button onPress={handleShowAddForm} className={secondaryBtnClass}>
            モデルを追加
          </Button>
        )}
      </section>
      ...
      <section className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-5">
          <Select selectedKey={settings.model} onSelectionChange={handleModelChange}>
            ...
            <Popover className={popoverClass}>
              <ListBox>
                <ListBoxItem id="sonic-2" className={itemClass}>
                  sonic-2
                </ListBoxItem>
                <ListBoxItem id="sonic-3" className={itemClass}>
                  sonic-3
                </ListBoxItem>
              </ListBox>
            </Popover>
          ...
          <Select selectedKey={settings.language} onSelectionChange={handleLanguageChange}>
            ...
            <Popover className={popoverClass}>
              <ListBox>
                <ListBoxItem id="ja" className={itemClass}>
                  日本語
                </ListBoxItem>
                <ListBoxItem id="en" className={itemClass}>
                  English
                </ListBoxItem>
              </ListBox>
            </Popover>
        ...
        <Select selectedKey={String(settings.sampleRate)} onSelectionChange={handleSampleRateChange}>
          ...
          <Popover className={popoverClass}>
            <ListBox>
              <ListBoxItem id="22050" className={itemClass}>
                22050 Hz
              </ListBoxItem>
              <ListBoxItem id="44100" className={itemClass}>
                44100 Hz
              </ListBoxItem>
            </ListBox>
          </Popover>
